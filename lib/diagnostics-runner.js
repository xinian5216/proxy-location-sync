...
export function createDiagnosticsController({
  ...
  async function execute(opts, myRun) {
    const { state, settings, locale, environment, page, force } = opts;
  ...
  async function run({ state, settings, locale, environment, page, force = false, reason } = {}) {
    const detectedIp = detectedExitIp(state);
    ...
    const work = (async () => {
      try {
        return await execute(
          { state, settings, locale, environment, page, force: incomingForce },
          myRun,
        );
      } finally {
